...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($existingProduct) {
            // Product exists, now update the stock and stock_unit
            $existingStock = Stock::where('product_id', $existingProduct->product_id)
                ->where('stock_unit', $request->stock_unit)
                ->first();

            if ($existingStock) {
                // Update the stock quantity of the existing stock
                $existingStock->quantity += $request->stock; // Add the new stock to the existing quantity
                $existingStock->stock_unit = $request->stock_unit; // Update the stock unit
                ...
                return redirect()->route('products.index')
                    ->with('success', 'Stock updated successfully for existing product.');
            } else {
                // If no stock record exists for this product and stock_unit, create a new stock record
            ...
            // Log the initial stock in StockHistory
            StockHistory::create([
