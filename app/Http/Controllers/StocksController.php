...
class StocksController extends Controller
{
    ...
    public function update(Request $request, $productId)
    {
        ...
        $currentStock = $product->stock->quantity ?? 0; // Get the current stock quantity
        ...
        // Check if the stock out quantity exceeds the current stock
        if ($request->stockOutQuantity > $currentStock) {
            return redirect()->back()->withErrors([
                'stockOutQuantity' => "The stock out quantity must not exceed the current stock.",
            ]);
        }

        // Process the stock out logic here
        $newStock = $currentStock - $request->stockOutQuantity;

        // Update the stock
        $product->stock->update(['quantity' => $newStock]);

        // Add to stock history
        StockHistory::create([
            'stock_id' => $product->stock->stock_id,
    ...
    public function showStockHistory($productId)
    {
        ...
        // Retrieve the stock history with stock details
